...
# --- r1024v experiment switches (dev copy only; all OFF = ship v33) ---
PREFIX_REFINE = False       # mechanism A: when tmax < R <= PREFIX_T_CAP and
                            # grams are carried, refine only the FIRST
                            # PREFIX_ROWS rows (rows are independent in the
                            # exact objective, so any row subset is valid)
PREFIX_ROWS = 1024
PREFIX_T_CAP = 4096
PREFIX_SWEEPS = 0           # 0 = ladder tier of the prefix call (1024 rows
                            # -> 8 sweeps); >0 forces the sweep count
DYN_TMAX_OVERRIDE = None    # mechanism A "full" config: if int, the dynamic
                            # refine gate uses max(state tmax, this) so ALL
                            # rows refine at the ladder (low) tier
DYN_SELECT = 0              # mechanism B: 0 = ship return; 2 = exact per-call
                            # selection between the ship default and the
                            # table-only variant by J(v) = <v, v@gw - x@gwf>,
                            # the EXACT output MSE the lattice stage itself
                            # optimizes (same carried grams); 3 = additionally
                            # score the bare GPTQ variant when refinement ran
DYN_DEBUG = None            # harness injects a list; each dynamic call
                            # appends its candidate scores / selection
...
def _refine_act_values(x: torch.Tensor, values: torch.Tensor,
                       ...
                       gwf: torch.Tensor,
                       n_sweeps_override: int = 0) -> torch.Tensor:
    ...
    if n_sweeps_override > 0:
        n_sweeps = n_sweeps_override
    else:
...
def hif4_dynamic_quantize_activation(
    ...
    if isinstance(activation_state, dict):
        ...
        gw_t = activation_state.get("gw")
        gwf_t = activation_state.get("gwf")
    if DYN_TMAX_OVERRIDE is not None:
        tmax = max(tmax, int(DYN_TMAX_OVERRIDE))
    if not (isinstance(gw_t, torch.Tensor) and isinstance(gwf_t, torch.Tensor)
            and tuple(gw_t.shape) == (C, C) and tuple(gwf_t.shape) == (C, C)):
        gw_t = gwf_t = None
    v_ref = None
    v_pref = None
    if gw_t is not None:
        ...
        if R <= tmax:
            try:
                ...
                v_ref = _refine_act_values(x, v0, unit, gw_f, gwf_f)
            except Exception:
                v_ref = None
        elif (PREFIX_REFINE and R <= PREFIX_T_CAP):
            # mechanism A: rows are independent in the exact objective, so
            # refining only the first PREFIX_ROWS rows still improves those
            # rows' share of J by the full greedy descent
            try:
                v0 = values if values is not None else _deq_params(p)
                n = min(R, PREFIX_ROWS)
                v_head = _refine_act_values(
                    x[:n], v0[:n], unit[:n], gw_f, gwf_f,
                    n_sweeps_override=PREFIX_SWEEPS)
                v_pref = torch.cat([v_head, v0[n:]], dim=0)
            except Exception:
                v_pref = None
    # ---- ship default choice ----
    if v_ref is not None:
        default = ("ref", v_ref)
    elif v_pref is not None:
        default = ("pref", v_pref)
    elif values is not None:
        default = ("gptq", values)
    else:
        default = ("table", None)
    # ---- mechanism B: exact per-call variant selection ----
    dbg = None
    if isinstance(DYN_DEBUG, list):
        dbg = {"R": R, "C": C, "tmax": tmax,
               "g": int(isinstance(activation_state, dict)
                        and activation_state.get("g") == 1),
               "grams": int(gw_t is not None), "cands": {}, "sel": None}
        DYN_DEBUG.append(dbg)
    chosen = default
    if DYN_SELECT >= 2 and gw_t is not None and default[0] != "table":
        import time as _t
        _t0 = _t.perf_counter()
        cands = [default]
        if DYN_SELECT >= 3 and values is not None and default[0] != "gptq":
            cands.append(("gptq", values))
        cands.append(("table", _deq_params(p)))
        # J(v) = <v, v@gw> - 2<v, x@gwf> + const is the EXACT output MSE the
        # lattice objective optimizes (M = v@gw - x@gwf is its half-gradient;
        # the VALUE needs the factor-2 cross term); x@gwf is shared by all
        # candidates
        xg = x @ gwf_t.float()
        best_j = None
        best = cands[0]
        for tag, v in cands:
            j = (v * (v @ gw_t.float() - 2.0 * xg)).sum().item()
            if dbg is not None:
                dbg["cands"][tag] = j
        ...
        if dbg is not None:
            dbg["sel"] = chosen[0]
            dbg["sel_ms"] = (_t.perf_counter() - _t0) * 1e3
            # float64 ground truth of the same scores (measurement only;
            # excluded from timing runs because DYN_DEBUG is None there)
            gw64 = gw_t.double()
            xg64 = x.double() @ gwf_t.double()
            dbg["cands64"] = {}
            for tag, v in cands:
                v64 = v.double()
                dbg["cands64"][tag] = (v64 * (v64 @ gw64 - 2.0 * xg64)).sum().item()
    if dbg is not None:
        dbg["default"] = default[0]
        dbg["flipped"] = int(chosen[0] != default[0])
    if chosen[0] == "table":
        return p
    return _values_to_params(chosen[1].contiguous(), p)
