...
## pairflip verdict + 420s-era second wave - 2026-08-28
- pairflip: NO-SHIP -- collaborative flip pairs are REAL and everywhere (80-97% of rows have >=1) but tiny (|dJ| p50 0.005-0.017 vs full J 1e5-1e7); pure pair margin +0.127pp/case = 1/4 of SHIP line; cascade machinery costs 3-5x refine work for ~+25 ideal online. Search quality is NOT the next bottleneck. Ledger entry warranted. Byproducts: (1) v42 tiers were UNDER-CONVERGED in the fp32-gram era (+0.34pp/case to deep-converge at only 0.18-1.4s/call local) -- v43's 56/28/14 eats part; (2) synthetic outlier groups are HUGELY under-converged (+6.2/+19.2pp) -- if judge linear groups have an outlier population, tier depth matters an order of magnitude more.
- v43 pricing refinement: mini cal 9.1s dyn 6.7s (v43) -- well within 420s if confirmed. If judge has outlier-like groups, consider one more tier step (72/40/20) in v44 after reading v43's actual time consumption.
## v43 SECURITY INTERCEPT - 2026-08-28
- v43: REJECTED with [Security:LOCAL_FILE_IO] "contestant local file read/write operations are prohibited" + all 350 cases WA. Code audit: v42->v43 diff is two pure-numeric changes (tiers, T_MAX); solution.py contains ZERO file I/O. Not our code -- judge environment changed (new security scanner with the new test set / 420s rollout, likely).
- Control experiment: resubmit v42. If v42 also intercepted -> environment-wide policy, ALL recent packages dead, need to find what pattern trips it (could be zipfile metadata? could be scanner false positive on torch/numpy idioms). If v42 passes -> something about v43's zip specifically (unlikely given identical builder).
## LOCAL_FILE_IO intercept analysis - 2026-08-28 (cont)
- user confirms: not v43-specific, judge environment got stricter (all contestants affected era).
- deep pattern scan of v42 (zero real file I/O): 0 hits on open(/torch.load/save/pickle/os-import/__file__; "eval|exec" hits are only the words inside docstrings ("eval" in 'proxy eval', "exec" -- none are calls); 'with' hits are all English prose in docstrings, not with-statements; 'file-ish words' are variable names (tmp) and comments. Nothing a reasonable scanner flags. Either the scanner is cruder than reasonable (word-list based, would flag docstrings) or it intercepts at a different layer (zip metadata? runtime syscall audit? torch-internal temp files?).
- leading hypotheses ranked: (1) runtime syscall audit flagging torch/numpy internal temp/mmap behavior (env-level, hits everyone); (2) crude word-list scanner hitting docstrings/comments; (3) zip-layer metadata policy. Next: wait for user's community intel / official notice; build a 'scanner-friendly' probe only if we learn the trigger pattern.
## 0831公告 compliance audit - 2026-08-31
- Announcement: code review dropped (initial round); linear share reduced; timeout now 5 minutes (420s CONFIRMED); only .py uploads; banned module list: os pathlib io tempfile shutil subprocess socket ctypes pickle marshal importlib zipimport pkgutil runpy mmap resource pty tty sys builtins; ALL contestants must resubmit for new scoring.
- Our imports: __future__, typing, numpy, torch, threading -- ZERO banned, zero file I/O, zero .pt reads. Deep textual scan: no banned name in import/attribute contexts anywhere. Zip: single standard solution.py entry, clean metadata, identical across v40-v43.
- Conclusion: our code complies with every announced rule. The v43 [Security:LOCAL_FILE_IO] rejection is most plausibly platform rescan noise (torch internally imports banned names -- a naive transitive-import scanner would flag ANY torch submission; or rescan pipeline hiccup). ACTION: user resubmits v42 (or v43) per announcement; if rejection persists for all submissions, it is platform-side and must be reported with our audit as evidence.
- Pricing note: 420s confirmed -> v43 (56/28/14 + T2048, ~340s ordinary window est) is the right carrier; linear-share reduction slightly lowers linear-mechanism values but smooth/QKS/refine all still apply.
## narrowing the LOCAL_FILE_IO flag - 2026-08-31
- confirmed by user: other contestants HAVE scores -> torch/numpy imports do NOT trigger the scanner. Problem is in OUR file specifically.
