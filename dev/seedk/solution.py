...
# seedk experiment hooks: rotation sign-seed bases. Defaults are the shipped
# v33 values; the dev/seedk harness overrides these module globals to sweep
# seeds. No other behavior change.
_ROT_LIN_SEED_BASE = 777
_ROT_ATTN_SEED_BASE = 0xA5A5
...
def _rot_blocks(x: torch.Tensor) -> torch.Tensor:
    ...
    d = torch.empty(nb, 64)
    for b in range(nb):
        g = torch.Generator().manual_seed(_ROT_LIN_SEED_BASE + b)
...
_R_CACHE: dict[tuple[int, int], torch.Tensor | None] = {}
...
def _make_R(dh: int):
    ...
    seedk: cache is keyed by (dh, seed base) so harness sweeps can't go stale.
    """
    key = (dh, _ROT_ATTN_SEED_BASE)
    if key in _R_CACHE:
        return _R_CACHE[key]
    if dh & (dh - 1):
        _R_CACHE[key] = None
    ...
    while H.shape[0] < dh:
        H = torch.cat([torch.cat([H, H], 1), torch.cat([H, -H], 1)], 0)
    H = H / (dh ** 0.5)
    g = torch.Generator().manual_seed(_ROT_ATTN_SEED_BASE + dh)
    d = (torch.rand(dh, generator=g) < 0.5).float() * 2 - 1
    _R_CACHE[key] = H * d.unsqueeze(0)
    return _R_CACHE[key]
